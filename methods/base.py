...
class Base_Client():
    ...
    def run(self, received_info):
        ...
        for client_idx in self.client_map[self.round]:
            ...
            client_results.append({'weights':weights, 'num_samples':num_samples,'acc':acc,'train_acc':train_acc, 'client_index':self.client_index})
            ...
            for batch_idx, (images, labels) in enumerate(self.train_dataloader):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    self.loss_scaler(loss, self.optimizer, clip_grad=None,
                                ...
                                update_grad=(batch_idx+ 1) % accum_iter == 0, args=self.args, last_epoch=end_epoch-epoch, model=self.model, noise_multiplier=self.noise_multiplier, device=self.device)
                    #self.optimizer.step()
    ...
    def dp_train(self):
        misc.load_model(args=self.args, model_without_ddp=self.model, optimizer=self.optimizer, loss_scaler=self.loss_scaler)
        # train the local model
        self.model.to(self.device)
        self.model.train()
        # _writer = glo.get_value("writer")
...
class Base_Server():
    ...
    def log_info(self, client_info, acc, loss):
        ...
        if not self.args.debug:
            wandb.log({"Test/AccTop1": acc,"Test_Loss": loss, "Client_Train/AccTop1": client_tacc, "Client_Test/AccTop1": client_acc,"round": self.round})
